...
// Optional measurement of the RMC requests the client sends, so the emulator's SMM2 upload
// behavior can be diffed against the a console measured. Enabled by SMM2_CAPTURE=<file>;
...
func captureRMC(tag string, req *nex.RMCMessage) {
	...
	if captureFH == nil {
		...
		captureFH = f
		fmt.Printf("[SMM2 measured] recording client requests -> %s\n", captureFile)
	}
